...
lazy val uberJar = (project in file("hnswlib-spark"))
  .settings(
    ...
    noPublishSettings,
    crossScalaVersions := Seq("2.13.16", "2.13.16"),
    ...
    pyTest := {
      ...
      if (scalaVersion.value == "2.13.16" && sparkVersion.value >= "3.0.0" || scalaVersion.value == "2.11.12") {
        val ret = Process(
...
lazy val cosmetic = project
  .settings(
    ...
    autoScalaLibrary     := false,
    crossScalaVersions   := Seq("2.13.16", "2.13.10"),
    sparkVersion         := sys.props.getOrElse("sparkVersion", "3.5.5"),
    publishSettings
  )
