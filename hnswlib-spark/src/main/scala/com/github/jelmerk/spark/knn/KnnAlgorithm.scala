...
private[knn] trait KnnModelParams extends Params with HasFeaturesCol with HasPredictionCol {
  ...
  /** Param that indicates whether to not return the a candidate when it's identifier equals the query identifier
    ...
    * @group param
    */
  final val excludeSelf =
    new BooleanParam(this, "excludeSelf", "whether to include the row identifier as a candidate neighbor")
  ...
  /** Param for the threshold value for inclusion. -1 indicates no threshold Default: -1
    ...
    * @group param
    */
  final val similarityThreshold =
    new DoubleParam(this, "similarityThreshold", "do not return neighbors further away than this distance")
  ...
  /** Param that specifies the number of threads to use. Default: number of processors available to the Java virtual
    * machine
    *
    * @group param
    */
  ...
  /** Param for the output format to produce. One of "full", "minimal" Setting this to minimal is more efficient when
    * all you need is the identifier with its neighbors
    *
    * Default: "full"
    *
    * @group param
  ...
  protected def validateAndTransformSchema(schema: StructType, identifierDataType: DataType): StructType = {
    ...
    getOutputFormat match {
      case "minimal" if !isSet(queryIdentifierCol) =>
  ...
  override def load(path: String): TModel = {
    ...
    val model = (identifierType, vectorType) match {
      case ("int", "float_array") =>
        createModel[Int, Array[Float], IntFloatArrayIndexItem, Float](uid, indicesPath, partitions)
      case ("int", "double_array") =>
        createModel[Int, Array[Double], IntDoubleArrayIndexItem, Double](uid, indicesPath, partitions)
      case ("int", "vector") => createModel[Int, Vector, IntVectorIndexItem, Double](uid, indicesPath, partitions)

      case ("long", "float_array") =>
        createModel[Long, Array[Float], LongFloatArrayIndexItem, Float](uid, indicesPath, partitions)
      case ("long", "double_array") =>
        createModel[Long, Array[Double], LongDoubleArrayIndexItem, Double](uid, indicesPath, partitions)
      case ("long", "vector") => createModel[Long, Vector, LongVectorIndexItem, Double](uid, indicesPath, partitions)

      case ("string", "float_array") =>
        createModel[String, Array[Float], StringFloatArrayIndexItem, Float](uid, indicesPath, partitions)
      case ("string", "double_array") =>
        createModel[String, Array[Double], StringDoubleArrayIndexItem, Double](uid, indicesPath, partitions)
      case ("string", "vector") =>
        createModel[String, Vector, StringVectorIndexItem, Double](uid, indicesPath, partitions)
      case _ =>
...
private[knn] trait KnnModelOps[
  ...
  protected def typedTransform(dataset: Dataset[_])(implicit
      tId: TypeTag[TId],
      tVector: TypeTag[TVector],
      tDistance: TypeTag[TDistance],
      evId: ClassTag[TId],
      evVector: ClassTag[TVector],
      distanceNumeric: Numeric[TDistance]
  ): DataFrame = {
    ...
    if (isSet(queryIdentifierCol)) typedTransformWithQueryCol[TId](dataset, getQueryIdentifierCol)
      ...
      typedTransformWithQueryCol[Long](dataset.withColumn("_query_id", monotonically_increasing_id), "_query_id")
        .drop("_query_id")
  }

  protected def typedTransformWithQueryCol[TQueryId](dataset: Dataset[_], queryIdCol: String)(implicit
      tId: TypeTag[TId],
      tVector: TypeTag[TVector],
      tDistance: TypeTag[TDistance],
      tQueryId: TypeTag[TQueryId],
      evId: ClassTag[TId],
      evVector: ClassTag[TVector],
      evQueryId: ClassTag[TQueryId],
      distanceNumeric: Numeric[TDistance]
  ): DataFrame = {
    ...
    val serializableHadoopConfiguration = new SerializableConfiguration(
    ...
    val logicalPartitionAndQueries =
      if (isDefined(queryPartitionsCol))
        dataset
          .select(
      ...
      .mapPartitions { queriesWithPartition =>
        ...
        if (!fileSystem.exists(indexPath)) Iterator.empty
          ...
          logInfo(
            logicalPartitionId,
            replica,
            s"started loading index from $indexPath on host ${InetAddress.getLocalHost.getHostName}"
          )
          val index = loadIndex(fileSystem.open(indexPath))
          logInfo(
            logicalPartitionId,
            replica,
            s"finished loading index from $indexPath on host ${InetAddress.getLocalHost.getHostName}"
          )
          ...
          new Iterator[(TQueryId, Seq[Neighbor[TId, TDistance]])] {
            ...
            private[this] val queue =
              new LinkedBlockingQueue[(TQueryId, Seq[Neighbor[TId, TDistance]])](batchSize * numThreads)
            private[this] val executorService = new ThreadPoolExecutor(
              numThreads,
              numThreads,
              60L,
              TimeUnit.SECONDS,
              new LinkedBlockingQueue[Runnable],
              new NamedThreadFactory("searcher-%d")
            ) {
            ...
            override def next(): (TQueryId, Seq[Neighbor[TId, TDistance]]) = {
              if (first) {
                logInfo(
                  logicalPartitionId,
                  replica,
                  s"started querying on host ${InetAddress.getLocalHost.getHostName} with ${sys.runtime.availableProcessors} available processors."
                )
                first = false
              }
              ...
              if (!hasNext) {
                logInfo(
                  logicalPartitionId,
                  replica,
                  s"finished querying $count items on host ${InetAddress.getLocalHost.getHostName}"
                )
            ...
            class Worker(queries: Iterator[(TQueryId, TVector)], activeWorkers: CountDownLatch, batchSize: Int)
                extends Runnable {
              ...
              @tailrec final override def run(): Unit = {
                ...
                work.foreach { case (id, vector) =>
                  val neighbors = index
                    .findNearest(vector, fetchSize)
                    .collect {
                      case SearchResult(item, distance)
...
private[knn] class CleanupListener(dir: String, serializableConfiguration: SerializableConfiguration)
